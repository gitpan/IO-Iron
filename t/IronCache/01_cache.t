...
my $iron_cache_client = IO::Iron::IronCache::Client->new( { 'config' => 'iron_cache.json'} );
my $project_id = $iron_cache_client->{'connection'}->{'project_id'};
...
$log_message = "(project=$project_id, cache=$unique_cache_name_01). Cleared cache.";
map { $log_test = 1 if ($_->{level} eq 'info' 
		&& $_->{category} eq 'IO::Iron::Connection' 
		&& $_->{message} eq $log_message
	) } @{$log->msgs};
